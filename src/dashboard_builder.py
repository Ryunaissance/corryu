...
const tagColors = {{
    '배당':'bg-yellow-600 text-yellow-100','테크/IT':'bg-sky-600','헬스케어':'bg-rose-600 text-rose-100',
    '성장주':'bg-indigo-600','가치주':'bg-amber-600','중소형주':'bg-green-600','반도체':'bg-blue-500',
    'AI/로봇':'bg-purple-600 text-purple-100','메타버스':'bg-pink-600','금융':'bg-cyan-600',
    '에너지':'bg-orange-600','부동산':'bg-teal-600','ESG':'bg-emerald-600',
    '퀄리티':'bg-violet-600','모멘텀':'bg-fuchsia-600','저변동성':'bg-lime-600',
    '우주항공':'bg-slate-600','인프라':'bg-zinc-600','원자재':'bg-stone-600'
}};
    ...
    table = $('#masterTable').DataTable({{
        ...
        columns: [
            ...
            {{
                ...
                render: function(d, type, row) {{
                    ...
                    if (row.tags && row.tags.length) {{
                        h += '<div class="mt-0.5 flex flex-wrap gap-0.5">';
                        row.tags.forEach(function(t) {{
                            let c = tagColors[t] || 'bg-gray-600';
                            h += '<span class="tag '+c+' bg-opacity-80">'+t+'</span>';
                        }});
                        h += '</div>';
                    }}
            ...
            {{ data: 'isl', render: function(d,t,row) {{
                ...
                let reasons = (row.lr||[]).join(', ');
                let details = (row.ld||[]).join('\\n');
                return '<div class="tooltip-container"><span class="badge-legacy">Legacy</span>'
                    + '<div class="tooltip-content"><b>'+reasons+'</b><br>'+details.replace(/\\n/g,'<br>')+'</div></div>';
            }} }}
        ],
