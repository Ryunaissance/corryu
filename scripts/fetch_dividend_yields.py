#!/usr/bin/env python3
"""
yfinance로 전체 ETF 배당수익률 수집 스크립트

수집 대상: output/etf_data.json의 전체 티커 (~1,651개)
수집 방법: yfinance Ticker.info['dividendYield']
           없으면  Ticker.info['trailingAnnualDividendYield'] fallback
저장 위치:
    - data_scraped/dividend_yields.pkl  (Dict[str, float])  ← 로컬 빌드용
    - data_scraped/dividend_yields_raw.json  (체크포인트/디버그)
    - Supabase etf_dividends 테이블  ← CI 일별 워크플로우용 (env 있을 때만)

중단/재시작 지원:
    체크포인트를 dividend_yields_raw.json에 실시간 저장.
    재실행 시 이미 조회된 티커는 건너뛰고 이어서 처리.

실행 예시:
    python scripts/fetch_dividend_yields.py
    python scripts/fetch_dividend_yields.py --reset   # 체크포인트 무시 후 처음부터
"""
...
CHECKPOINT_JSON = os.path.join(DATA_SCRAPED, 'dividend_yields_raw.json')
OUTPUT_PKL      = os.path.join(DATA_SCRAPED, 'dividend_yields.pkl')
...
def upsert_to_supabase(results: dict[str, float | None]):
    """Supabase etf_dividends 테이블에 upsert (env 변수가 있을 때만)"""
    url = os.environ.get('SUPABASE_URL', '')
    key = os.environ.get('SUPABASE_SERVICE_KEY', '')
    if not url or not key:
        log.info('Supabase 환경변수 없음 — 로컬 pkl만 저장')
        return
    ...
    try:
        ...
        log.warning('Supabase 연결 실패: %s', e)
        return

    rows = [
        {'ticker': t, 'dividend_yield': v}
    ...
    for i in range(0, len(rows), BATCH_DB):
        ...
        try:
            client.table('etf_dividends').upsert(
                chunk, on_conflict='ticker'
            ).execute()
        except Exception as e:
            log.warning('Supabase upsert 오류 (batch %d): %s', i // BATCH_DB + 1, e)

    log.info('Supabase etf_dividends에 %d개 upsert 완료', len(rows))
...
def main():
    ...
    for i, ticker in enumerate(todo, 1):
        ...
        if val is not None:
            found += 1
            log.info('[%d/%d] %-10s %.2f%%', i, len(todo), ticker, val * 100)
        else:
        ...
        if i % CHECKPOINT_EVERY == 0:
            ...
            log.info('--- 체크포인트 저장 %d/%d  (배당수익률 확보: %d개) ---',
    ...
    total = len(tickers)
    log.info('완료: %d/%d개 배당수익률 확보 (%.1f%%)', len(clean), total,
             len(clean) / total * 100 if total else 0)
    ...
    if missing:
        log.info('배당수익률 없는 티커 %d개: %s ...', len(missing), missing[:20])
