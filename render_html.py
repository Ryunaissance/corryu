...
#gh-modal {{ display:none; position:fixed; inset:0; z-index:10001; background:rgba(0,0,0,0.72); backdrop-filter:blur(4px); align-items:center; justify-content:center; }}
#gh-modal.show {{ display:flex; }}
#gh-modal-card {{ background:#0f1623; border:1px solid rgba(59,130,246,0.4); border-radius:16px; padding:24px; width:min(480px,95vw); box-shadow:0 24px 64px rgba(0,0,0,0.7); }}
.gh-input {{ background:rgba(20,24,38,0.95); border:1px solid rgba(255,255,255,0.1); border-radius:8px; color:#e2e8f0; padding:8px 12px; font-size:0.85rem; width:100%; font-family:monospace; box-sizing:border-box; }}
...
<!-- GitHub 동기화 설정 모달 -->
<div id="gh-modal">
    <div id="gh-modal-card">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:18px">
            <div>
                <div style="font-weight:700;font-size:1rem;color:#e2e8f0">⚙ GitHub 동기화 설정</div>
                <div style="font-size:0.78rem;color:#64748b;margin-top:3px">PAT을 저장하면 레거시·카테고리 설정이 모든 기기에 자동 동기화됩니다</div>
            </div>
            <button id="gh-modal-close" style="background:transparent;border:none;color:#475569;font-size:1.3rem;cursor:pointer;line-height:1;padding:2px 6px">×</button>
        </div>
        <div style="margin-bottom:14px">
            <label style="font-size:0.8rem;color:#94a3b8;display:block;margin-bottom:5px">GitHub Personal Access Token (repo 권한 필요)</label>
            <input type="password" id="gh-pat-input" class="gh-input" placeholder="github_pat_... 또는 ghp_...">
            <div style="font-size:0.72rem;color:#475569;margin-top:6px">Settings → Developer settings → Personal access tokens → <b style="color:#94a3b8">Contents: Read &amp; write</b> 권한 필요</div>
        </div>
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            <button id="gh-pat-save" style="padding:8px 18px;border-radius:8px;background:rgba(59,130,246,0.2);color:#93c5fd;border:1px solid rgba(59,130,246,0.4);font-size:0.82rem;font-weight:600;cursor:pointer">저장 &amp; 연결</button>
            <button id="gh-pat-clear" style="padding:8px 14px;border-radius:8px;background:rgba(239,68,68,0.1);color:#f87171;border:1px solid rgba(239,68,68,0.2);font-size:0.82rem;font-weight:600;cursor:pointer">연결 해제</button>
            <span id="gh-test-result" style="font-size:0.78rem;color:#64748b"></span>
        </div>
    </div>
</div>
...
// ── GitHub 다기기 동기화 ──────────────────────────────────
const GH_REPO   = 'Ryunaissance/corryu';
const GH_BRANCH = 'main';
const GH_PATH   = 'output/user_overrides.json';
const GH_PAT_KEY = 'corryu_gh_pat';

function getGhPat() {{ return localStorage.getItem(GH_PAT_KEY) || ''; }}
function saveGhPat(pat) {{
    if (pat) localStorage.setItem(GH_PAT_KEY, pat);
...
async function fetchRemoteOverrides() {{
    try {{
        const url = 'https://raw.githubusercontent.com/' + GH_REPO + '/' + GH_BRANCH + '/' + GH_PATH + '?t=' + Date.now();
...
async function pushRemoteOverrides(ov) {{
    ...
    setSyncStatus('busy');
    try {{
        const payload = Object.assign({{ _meta: {{ ts: Date.now() }} }}, ov);
        ...
        if (!_ghSha) {{
            ...
            if (gr.ok) {{ const gd = await gr.json(); _ghSha = gd.sha; }}
            else if (gr.status !== 404) {{ setSyncStatus('err'); return; }}
        }}
        const body = {{ message: 'sync: corryu user overrides', content, branch: GH_BRANCH }};
        ...
        if (pr.ok) {{
            ...
            _ghSha = pd.content && pd.content.sha;
            setSyncStatus('ok');
...
function initDashboard() {{
    ...
    $('#sync-status').on('click', function() {{
        const pat = getGhPat();
        $('#gh-pat-input').val('').attr('placeholder', pat ? '변경하려면 새 PAT를 입력하세요' : 'github_pat_... 또는 ghp_...');
        $('#gh-test-result').text(pat ? '현재 PAT 설정됨' : '');
        $('#gh-modal').addClass('show');
    }});
    $('#gh-modal-close').on('click', function() {{ $('#gh-modal').removeClass('show'); }});
    $('#gh-modal').on('click', function(e) {{ if (e.target === this) $(this).removeClass('show'); }});
    $('#gh-pat-save').on('click', async function() {{
        const pat = $('#gh-pat-input').val().trim();
        if (!pat) {{ $('#gh-test-result').text('PAT을 입력하세요'); return; }}
        $('#gh-test-result').text('연결 테스트 중…');
        try {{
            ...
            if (r.ok) {{
                ...
                _ghSha = null;
                setSyncStatus('ok');
                $('#gh-test-result').css('color', '#4ade80').text('✓ 연결 성공! 다음 저장 시 자동 동기화됩니다');
                pushRemoteOverrides(getUserOverrides());
            }} else {{
                $('#gh-test-result').css('color', '#f87171').text('✗ 연결 실패 (' + r.status + ') — 토큰 권한을 확인하세요');
                setSyncStatus('err');
            }}
        }} catch(e) {{
            $('#gh-test-result').css('color', '#f87171').text('✗ 네트워크 오류');
            setSyncStatus('err');
        }}
    }});
    $('#gh-pat-clear').on('click', function() {{
        saveGhPat('');
        _ghSha = null;
        setSyncStatus('off');
        $('#gh-pat-input').val('').attr('placeholder', 'github_pat_... 또는 ghp_...');
        $('#gh-test-result').css('color', '#64748b').text('연결 해제됨');
    }});
